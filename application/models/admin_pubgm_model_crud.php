...
class Admin_pubgm_model_crud extends CI_Model {
	...
	public function hapusdataPembayaran($table_name,$id)
	{
		$this->db->where('idTurnamen',$id);
		$this->db->set('is_active', 0);
		$hapusPembayaran = $this->db->update($table_name);
	...
	public function tambahdatabaganPUBG($table_name, $data)
	{
		$tambahBagan = $this->db->insert($table_name,$data);
		return $tambahBagan;
	}
	
	public function tambahdataTim($table_name, $data)
	{
		$tambahTim = $this->db->insert($table_name,$data);
		return $tambahTim;
	}
	...
	public function datapubgEdit($table_name,$id)
	{
		$this->db->where('idBagan',$id);
		$get_user = $this->db->get($table_name);
		return $get_user->row();
	...
	public function get_relasiSchedule()
	{
		$this->db->select(['a.*', 'b.*']) ;
		$this->db->from('tournament_regis a');
		$this->db->where('b.idGame', 1);
		$this->db->where('b.is_active', 1);
		$this->db->join('bagan_pubg b', 'b.namaTurnamen = a.namaTurnamen', 'b.namaPanitia = a.namaPanitia', 'left');
		// $this->db->order_by('namaTurnamen', 'asc');
		$return = $this->db->get('');
		return $return->result();
	}

	public function baganFinalPUBG($query) //PERHATIKAN getQuery
	{
		$get_user = $this->db->query($query);
		return $get_user->result_array();
	}
	
	public function get_relasi_dataFinal($id, $namatim)
	{
		$this->db->select('*');
		$this->db->from('bagan_pubg');
		$this->db->where('bagan_pubg.namaTim', $namatim);
        $this->db->join('tournament_regis', 'tournament_regis.idTurnamen = bagan_pubg.idTurnamen', 'left');
		// $this->db->order_by('namaTurnamen', 'asc');
		$return = $this->db->get('');
		return $return->result_array();
	}

	public function datashowFinal($table_name,$id)
	{
		$this->db->where('idTurnamen',$id);
		$get_user = $this->db->get($table_name);
		return $get_user->result_array();
	...
	public function transferkanData($table_name,$id)
	{
		$this->db->where('idTurnamen',$id);
		$get_user = $this->db->get($table_name);
		return $get_user->result_array();
	...
	public function datashowdataHidden($table_name,$id)
	{
		$this->db->where('idTurnamen',$id);
		$get_user = $this->db->get($table_name);
		return $get_user->row();
	...
	public function getnamaTim($id)
	{
		// $this->db->where('idTurnamen',$id);
		$query = $this->db->query('SELECT * FROM tournament_regis ORDER BY namaTim');
		return $query->result();
	}

	public function tambahdatatimPUBG($table_name, $data)
	{
		$tambahtimPUBG = $this->db->insert($table_name,$data);
		return $tambahtimPUBG;
	}

	public function ambildatayangdibutuhkan($table)
	{
    	$result = $this->db->get_where($table);
    	return $result->result_array();
	}

	public function hapusTim($table_name,$id)
	{
		$this->db->where('idbaganPUBG',$id);
		$hapusnamaTim = $this->db->delete($table_name);
		return $hapusnamaTim;
	}
}
